...
class EfficientTAMVideoPredictor(EfficientTAMBase):
    ...
    def propagate_in_video(
        ...
        Early-stop is enabled by default: if *every* object has NO prediction
        (no pixel > thr_logit) for >= kill_after_gap consecutive frames, stop early.
        ...
        # -------- early-stop config (defaults) --------
        _es_cfg      = inference_state.get("early_stop_cfg", {}) or {}
        _es_enabled  = bool(_es_cfg.get("enabled", True))
        _es_thr      = float(_es_cfg.get("thr_logit", 0.0))
        _es_kill_gap = int(_es_cfg.get("kill_after_gap", 15))
        _es_tail_pad = int(_es_cfg.get("tail_pad", 0))
        # ---------------------------------------------
        ...
        # NOTE: disable=not show_progress ties the UI to the new arg
        for frame_idx in tqdm(processing_order, desc="propagate in video", disable=not show_progress):
            ...
            # presence check (size-agnostic): at least one pixel > thr_logit
            if _es_enabled:
                any_present = False
                for j in range(batch_size):
                    mj = video_res_masks[j]
                    if mj.dim() == 3:
                        if mj.size(0) == 1:
                            mj = mj.squeeze(0)          # [1,H,W] -> [H,W]
                        else:
                            mj = mj.max(dim=0).values   # [C,H,W] -> [H,W]
                    has_pred = torch.count_nonzero(mj > _es_thr).item() > 0
                    if has_pred:
                        gaps[j] = 0
                        any_present = True
                    else:
                        gaps[j] += 1
                tail_counter = 0 if any_present else (tail_counter + 1)

            # yield this frame
            yield frame_idx, obj_ids, video_res_masks

            # early-stop after emitting
            if _es_enabled:
                all_dead = all(g >= _es_kill_gap for g in gaps)
                if all_dead and tail_counter >= _es_tail_pad:
                    break
